...
describe("community recipe access", () => {
  test("saveRecipe refuses to copy a private recipe from another household", async () => {
    const t = newTest();
    const alice = await createUserWithHousehold(t, "Alice");
    const mallory = await createUserWithHousehold(t, "Mallory");
    const privateRecipe = await createRecipe(t, alice.householdId, "private");

    await expect(
      mallory.asUser.mutation(api.community.saveRecipe, {
        recipeId: privateRecipe,
      }),
    ).rejects.toThrow("Recipe not publicly available");
  });

  test("saveRecipe copies a public recipe", async () => {
    const t = newTest();
    const alice = await createUserWithHousehold(t, "Alice");
    const bob = await createUserWithHousehold(t, "Bob");
    const publicRecipe = await createRecipe(t, alice.householdId, "public");

    const result = await bob.asUser.mutation(api.community.saveRecipe, {
      recipeId: publicRecipe,
    });
    expect(result.savedRecipeId).toBeDefined();

    const copy = await t.run(async (ctx) => ctx.db.get(result.savedRecipeId));
    expect(copy?.householdId).toEqual(bob.householdId);
    expect(copy?.visibility).toEqual("private");
  });

  test("likeRecipe refuses a private recipe from another household", async () => {
    const t = newTest();
    const alice = await createUserWithHousehold(t, "Alice");
    const mallory = await createUserWithHousehold(t, "Mallory");
    const privateRecipe = await createRecipe(t, alice.householdId, "private");
    ...
    await expect(
      mallory.asUser.mutation(api.community.likeRecipe, {
        recipeId: privateRecipe,
      }),
    ).rejects.toThrow("Recipe not publicly available");
  });

  test("likeRecipe works on public recipes", async () => {
    const t = newTest();
    const alice = await createUserWithHousehold(t, "Alice");
    const bob = await createUserWithHousehold(t, "Bob");
    const publicRecipe = await createRecipe(t, alice.householdId, "public");

    const result = await bob.asUser.mutation(api.community.likeRecipe, {
      recipeId: publicRecipe,
    });
...
describe("auth repair", () => {
  test("repairPasswordAccountByEmail restores an orphaned password account", async () => {
    const t = newTest();
    // Simulate a migration orphan: an authAccount whose user row is gone.
    const accountId = await t.run(async (ctx) => {
      const ghostUserId = await ctx.db.insert("users", {
      ...
      await ctx.db.delete(ghostUserId);
      return accountId;
    });

    const result = await t.mutation(api.users.repairPasswordAccountByEmail, {
      email: "ghost@example.com",
    });
    expect(result).toEqual({ repaired: true });

    const repairedUser = await t.run(async (ctx) => {
      const account = await ctx.db.get(accountId);
      return account ? await ctx.db.get(account.userId) : null;
    });
    expect(repairedUser).not.toBeNull();
    expect(repairedUser?.email).toEqual("ghost@example.com");
  });

  test("repairPasswordAccountByEmail is rate limited per email", async () => {
    const t = newTest();
    for (let i = 0; i < 10; i++) {
      await t.mutation(api.users.repairPasswordAccountByEmail, {
        email: "brute@example.com",
      });
    }
    // The 11th call inside the window silently no-ops.
    const result = await t.mutation(api.users.repairPasswordAccountByEmail, {
      email: "brute@example.com",
    });
