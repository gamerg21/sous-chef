...
test('portable packages must match platform, official URL and a GitHub SHA-256 digest', () => {
  const asset = { name: 'sous-chef-linux-arm64.tar.gz', browser_download_url: `${RELEASES_URL}/download/v0.10.0/sous-chef-linux-arm64.tar.gz`, digest: `sha256:${'a'.repeat(64)}` };
  expect(portableAsset({ version: '0.10.0', assets: [asset] }, 'linux', 'arm64').sha256).toHaveLength(64);
  for (const bad of [{ ...asset, digest: null }, { ...asset, browser_download_url: 'https://evil.example/file' }]) expect(() => portableAsset({ version: '0.10.0', assets: [bad] }, 'linux', 'arm64')).toThrow('verified portable');
  expect(() => portableAsset({ version: '0.10.0', assets: [asset] }, 'linux', 'x64')).toThrow();
});
