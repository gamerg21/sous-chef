...
5. **Completed: unavailable extensions out of the core journey.** Extensions left the main navigation; the catalog and listing pages are labeled as a preview with no Install, Enable, Connect, or Manage controls and no placeholder reviews. The install mutation and integration connect action reject with a clear message until an adapter exists, covered by tests. AI provider settings moved to Settings → AI settings, reachable with household members from the account menu. Community copy states that sharing stays within this instance. Grocery/calendar integrations and cross-instance sharing remain future work.
6. **In progress: home-server release acceptance.** A one-command Docker path now exists (`./homelab.sh up`: bundled Convex backend, keygen sidecar, idempotent setup job, web app; see DEPLOYMENT.md). Still needed: Fresh Docker checkout, trusted HTTPS from a real phone, barcode camera permission/device selection, verified email delivery, household invitations, and data/storage restore drill. Document and test the self-hosted Convex backend path on real infrastructure before calling it one-click.
